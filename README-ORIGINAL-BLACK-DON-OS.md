...
4. **Clone and run the Black Don OS installer**:
   ```bash
   git clone https://gitlab.com/theblackdon/rockon-os
   cd rockon-os
   ./install.sh
   ```
...
```bash
~/rockon-os/hosts/YOUR-HOSTNAME/variables.nix
```
...
sudo nixos-rebuild switch --flake ~/rockon-os#YOUR-HOSTNAME --show-trace
```
